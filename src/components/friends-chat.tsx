...
import { friendsChat } from "@/ai/flows/friends-chat-flow";
...
export function FriendsChat() {
  ...
  React.useEffect(() => {
    ...
    const storedFriendName = localStorage.getItem("friendName");
    ...
    if (storedFriendName) {
      ...
      setTempFriendName(storedFriendName);
        ...
        localStorage.setItem("friendName", defaultFriendName)
    }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await friendsChat({
        message: input,
        userName: userName,
  ...
  const handleSettingsSave = () => {
    ...
    setFriendName(tempFriendName);
    localStorage.setItem("userName", tempUserName);
    localStorage.setItem("friendName", tempFriendName);
          ...
          <ScrollArea className="h-full" viewportRef={viewportRef}>
            <div className="p-4 space-y-4">
              {messages.map((message) => (
                <ChatMessage key={message.id} message={message} botName={friendName} userName={userName} />
              ))}
              {isResponding && <TypingIndicator botIcon={<Users className="h-4 w-4" />} />}
            </div>
          </ScrollArea>
        </CardContent>
      ...
      <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Change your name and your friend's name here.
            </DialogDescription>
          </DialogHeader>
